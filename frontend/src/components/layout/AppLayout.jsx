...
export default function AppLayout() {
  ...
  const getNavItems = (role) => {
    const items = [{ icon: LayoutDashboard, label: "Dashboard", path: "/dashboard" }];

    switch (role) {
      case "Fleet Manager":
        items.push({ icon: Truck, label: "Vehicles", path: "/vehicles" });
        items.push({ icon: Wrench, label: "Maintenance", path: "/maintenance" });
        items.push({ icon: FileBarChart, label: "Reports", path: "/reports" });
        break;
      case "Dispatcher":
        items.push({ icon: Map, label: "Trips", path: "/trips" });
      ...
      case "Financial Analyst":
        items.push({ icon: Droplet, label: "Fuel Logs", path: "/fuel" });
        items.push({ icon: IndianRupee, label: "Expenses", path: "/expenses" });
        items.push({ icon: FileBarChart, label: "Reports", path: "/reports" });
        break;
      case "Admin":
        ...
        items.push({ icon: Map, label: "Trips", path: "/trips" });
        items.push({ icon: Wrench, label: "Maintenance", path: "/maintenance" });
        items.push({ icon: Droplet, label: "Fuel", path: "/fuel" });
        items.push({ icon: IndianRupee, label: "Expenses", path: "/expenses" });
        items.push({ icon: FileBarChart, label: "Reports", path: "/reports" });
          ...
          <div className="flex items-center gap-3 lg:gap-5">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:block text-right">
                <div className="text-sm font-medium text-warm-700 leading-tight">{user.name}</div>
                <div className="text-xs text-warm-500">{user.role}</div>
              </div>
